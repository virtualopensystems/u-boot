 ...
 */
static int nd2057_i2c_write(unsigned int reg_addr, unsigned value)
{
	uint8_t buff[2];
...
/**
 * Read a halfword from the nd2057
 *
 * @param	reg_addr	register address to read
 * @param	value		address to store the value to be read
 * @return	0 on success, non-0 on failure
 */
static int nd2057_i2c_read(unsigned int reg_addr, unsigned *value)
{
	uint8_t buff[2];
 ...
 * @return	0 on success, non-0 on failure
 */
static int nd2057_i2c_read_string(unsigned int reg_addr, char *str,
				  int maxlen)
{
	uint8_t len;
...
static int nd2057_select(void)
{
	...
	if (config.old_bus != config.bus) {
		...
		if (ret) {
			debug("%s: Cannot select NS2057, err %d\n",
			      __func__, ret);
			return -1;
		}
	}
...
static int nd2057_deselect(void)
{
	int ret;
...
int nd2057_get_temperature(int *milli_degrees_c)
{
	...
	if (nd2057_select())
		return -1;
	ret = nd2057_i2c_read(REG_TEMPERATURE, &temp);
	nd2057_deselect();
	if (ret)
		return -1;
...
int nd2057_get_info(struct battery_info *info)
{
	...
	if (nd2057_select())
		return -1;
	err = nd2057_i2c_read_string(REG_MANUF_NAME, info->manuf_name,
				    sizeof(info->manuf_name));
	err |= nd2057_i2c_read_string(REG_DEVICE_NAME, info->device_name,
				    sizeof(info->manuf_name));
	err |= nd2057_i2c_read_string(REG_DEVICE_CHEM, info->device_chem,
				    sizeof(info->manuf_name));
	err |= nd2057_i2c_read(REG_SERIAL_NUM, &info->serial_num);
	err |= nd2057_i2c_read(REG_DESIGN_CAP, &info->design_cap_mah);
	err |= nd2057_i2c_read(REG_DESIGN_VOLTAGE, &info->design_voltage_mv);
	err |= nd2057_i2c_read(REG_VOLTAGE, &info->voltage_mv);
	err |= nd2057_i2c_read(REG_AVERAGE_CURRENT, &value);
	info->average_current_ma = (short)value;
	err |= nd2057_i2c_read(REG_RELATIVE_CHARGE, &info->relative_charge);
	err |= nd2057_i2c_read(REG_BATTERY_STATUS, &info->battery_status);
	nd2057_deselect();
...
int nd2057_get_status(void)
{
	...
	if (nd2057_select())
		return -1;
	err = nd2057_i2c_read(REG_BATTERY_STATUS, &status);
	nd2057_deselect();
