...
/*
 * Set APLL values to alter ARM frequency
 *
 * @param new_freq_level	enum cpufreq_level, states new frequency
 * @param clk			struct exynos5_clock *,
 *				provides clock reg addresses
 */
static void exynos5250_set_apll(enum cpufreq_level new_freq_level,
					struct exynos5_clock *clk)
{
 ...
 */
static int exynos5250_set_voltage(enum cpufreq_level new_volt_index)
{
	int error;
...
int exynos5250_set_frequency(enum cpufreq_level new_freq_level)
{
	...
	if (old_freq_level < new_freq_level) {
		...
		error = exynos5250_set_voltage(new_freq_level);

		/* Change the system clock divider values */
		exynos5250_set_clkdiv(new_freq_level, clk);

		/* Change the apll m,p,s value */
		exynos5250_set_apll(new_freq_level, clk);
...
int exynos5250_cpufreq_init(const void *blob)
{
	...
	/* Get the node from FDT for DMC */
	node = fdtdec_next_compatible(blob, 0,
				      COMPAT_SAMSUNG_EXYNOS_DMC);
	if (node < 0) {
		debug("%s: No node for DMC in device tree\n", __func__);
		return -1;
	}

	/* Get default ARM frequency */
	val = fdtdec_get_int(blob, node, "arm-frequency", -1);
	if (val <= 0) {
		debug("%s: Missing ARM frequency definition\n", __func__);
		return -1;
	}

	/* Calculate default ARM frequence level */
	old_freq_level = (val / 100000000) - 2;
